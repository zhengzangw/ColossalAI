    ...
    with strategy.model_init_context():
        if args.model == 'gpt2':
            ...
            if args.model_name.startswith("dolly"):
                actor = AutoModelForCausalLM.from_pretrained(args.model_path, torch_dtype=torch.bfloat16).to(torch.cuda.current_device())
            elif args.model_name.startswith("chatglm"):
                actor = AutoModel.from_pretrained(args.model_path, trust_remote_code=True).to(torch.float16).to(torch.cuda.current_device())
                ...
                actor = AutoModelForCausalLM.from_pretrained(args.model_path, config=config, trust_remote_code=True, torch_dtype=torch.bfloat16).to(torch.cuda.current_device())
            elif args.model_name.startswith("fastchat-t5"):
                actor = AutoModelForSeq2SeqLM.from_pretrained(args.model_path, torch_dtype=torch.float16).to(torch.float16).to(torch.cuda.current_device())
                ...
                actor = GPTNeoXForCausalLM.from_pretrained(args.model_path, torch_dtype=torch.float16).to(torch.cuda.current_device())
